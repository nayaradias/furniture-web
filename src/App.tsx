import HeartIcon from 'assets/icons/heart';
import Avatar from 'components/Avatar';
import React from 'react';
import Button from './components/Button';

const App: React.FC = () => (
  <>
    {/* <Button variation="primary" iconPosition="none" title="TEste" />
    <div style={{ height: 10 }} />
    <Button variation="primary" iconPosition="right" title="TEste" />
    <div style={{ height: 10 }} />
    <Button variation="primary" iconPosition="left" title="TEste" />
    <div style={{ height: 10 }} />
    <Button variation="primary" iconPosition="centerRight" title="TEste" />
    <div style={{ height: 10 }} />
    <Button variation="primary" icon={<HeartIcon />} iconPosition="centerleft" title="TEste" />
    <div style={{ height: 10 }} />
    <Button variation="primary" disabled iconPosition="none" title="TEste" />
    <div style={{ height: 10 }} />
    <Button variation="primary" disabled iconPosition="right" title="TEste" />
    <div style={{ height: 10 }} />
    <Button variation="primary" disabled iconPosition="left" title="TEste" />
    <div style={{ height: 10 }} />
    <Button variation="primary" disabled iconPosition="centerRight" title="TEste" />
    <div style={{ height: 10 }} />
    <Button variation="primary" disabled icon={<HeartIcon />} iconPosition="centerleft" title="TEste" />
    <div style={{ height: 10 }} />

    <Button variation="secundary" iconPosition="none" title="TEste" />
    <div style={{ height: 10 }} />
    <Button variation="secundary" iconPosition="right" title="TEste" />
    <div style={{ height: 10 }} />
    <Button variation="secundary" iconPosition="left" title="TEste" />
    <div style={{ height: 10 }} />
    <Button variation="secundary" iconPosition="centerRight" title="TEste" />
    <div style={{ height: 10 }} />
    <Button variation="secundary" icon={<HeartIcon />} iconPosition="centerleft" title="TEste" />
    <div style={{ height: 10 }} />
    <Button variation="secundary" disabled iconPosition="none" title="TEste" />
    <div style={{ height: 10 }} />
    <Button variation="secundary" disabled iconPosition="right" title="TEste" />
    <div style={{ height: 10 }} />
    <Button variation="secundary" disabled iconPosition="left" title="TEste" />
    <div style={{ height: 10 }} />
    <Button variation="secundary" disabled iconPosition="centerRight" title="TEste" />
    <div style={{ height: 10 }} />
    <Button variation="secundary" disabled icon={<HeartIcon />} iconPosition="centerleft" title="TEste" />
    <div style={{ height: 10 }} />

    <Button variation="outline" iconPosition="none" title="TEste" />
    <div style={{ height: 10 }} />
    <Button variation="outline" iconPosition="right" title="TEste" />
    <div style={{ height: 10 }} />
    <Button variation="outline" iconPosition="left" title="TEste" />
    <div style={{ height: 10 }} />
    <Button variation="outline" iconPosition="centerRight" title="TEste" />
    <div style={{ height: 10 }} />
    <Button variation="outline" icon={<HeartIcon />} iconPosition="centerleft" title="TEste" />
    <div style={{ height: 10 }} />
    <Button variation="outline" disabled iconPosition="none" title="TEste" />
    <div style={{ height: 10 }} />
    <Button variation="outline" disabled iconPosition="right" title="TEste" />
    <div style={{ height: 10 }} />
    <Button variation="outline" disabled iconPosition="left" title="TEste" />
    <div style={{ height: 10 }} />
    <Button variation="outline" disabled iconPosition="centerRight" title="TEste" />
    <div style={{ height: 10 }} />
    <Button variation="outline" disabled icon={<HeartIcon />} iconPosition="centerleft" title="TEste" />
    <div style={{ height: 10 }} />

    <Button variation="ghost" iconPosition="none" title="TEste" />
    <div style={{ height: 10 }} />
    <Button variation="ghost" iconPosition="right" title="TEste" />
    <div style={{ height: 10 }} />
    <Button variation="ghost" iconPosition="left" title="TEste" />
    <div style={{ height: 10 }} />
    <Button variation="ghost" iconPosition="centerRight" title="TEste" />
    <div style={{ height: 10 }} />
    <Button variation="ghost" icon={<HeartIcon />} iconPosition="centerleft" title="TEste" />
    <div style={{ height: 10 }} />
    <Button variation="ghost" disabled iconPosition="none" title="TEste" />
    <div style={{ height: 10 }} />
    <Button variation="ghost" disabled iconPosition="right" title="TEste" />
    <div style={{ height: 10 }} />
    <Button variation="ghost" disabled iconPosition="left" title="TEste" />
    <div style={{ height: 10 }} />
    <Button variation="ghost" disabled iconPosition="centerRight" title="TEste" />
    <div style={{ height: 10 }} />
    <Button variation="ghost" disabled icon={<HeartIcon />} iconPosition="centerleft" title="TEste" />
    <div style={{ height: 10 }} />




    <div style={{ width: 176 }}>
      <div style={{ height: 100 }} />
      <Button variation="primary" iconPosition="none" title="TEste" small />
      <div style={{ height: 10 }} />
      <Button variation="primary" iconPosition="right" title="TEste" small />
      <div style={{ height: 10 }} />
      <Button variation="primary" iconPosition="left" title="TEste" small />
      <div style={{ height: 10 }} />
      <Button variation="primary" iconPosition="centerRight" title="TEste" small />
      <div style={{ height: 10 }} />
      <Button variation="primary" icon={<HeartIcon />} iconPosition="centerleft" title="TEste" small />
      <div style={{ height: 10 }} />
      <Button variation="primary" disabled iconPosition="none" title="TEste" small />
      <div style={{ height: 10 }} />
      <Button variation="primary" disabled iconPosition="right" title="TEste" small />
      <div style={{ height: 10 }} />
      <Button variation="primary" disabled iconPosition="left" title="TEste" small />
      <div style={{ height: 10 }} />
      <Button variation="primary" disabled iconPosition="centerRight" title="TEste" small />
      <div style={{ height: 10 }} />
      <Button variation="primary" disabled icon={<HeartIcon />} iconPosition="centerleft" title="TEste" small />
      <div style={{ height: 10 }} />

      <Button variation="secundary" iconPosition="none" title="TEste" small />
      <div style={{ height: 10 }} />
      <Button variation="secundary" iconPosition="right" title="TEste" small />
      <div style={{ height: 10 }} />
      <Button variation="secundary" iconPosition="left" title="TEste" small />
      <div style={{ height: 10 }} />
      <Button variation="secundary" iconPosition="centerRight" title="TEste" small />
      <div style={{ height: 10 }} />
      <Button variation="secundary" icon={<HeartIcon />} iconPosition="centerleft" title="TEste" small />
      <div style={{ height: 10 }} />
      <Button variation="secundary" disabled iconPosition="none" title="TEste" small />
      <div style={{ height: 10 }} />
      <Button variation="secundary" disabled iconPosition="right" title="TEste" small />
      <div style={{ height: 10 }} />
      <Button variation="secundary" disabled iconPosition="left" title="TEste" small />
      <div style={{ height: 10 }} />
      <Button variation="secundary" disabled iconPosition="centerRight" title="TEste" small />
      <div style={{ height: 10 }} />
      <Button variation="secundary" disabled icon={<HeartIcon />} iconPosition="centerleft" title="TEste" small />
      <div style={{ height: 10 }} />

      <Button variation="outline" iconPosition="none" title="TEste" small />
      <div style={{ height: 10 }} />
      <Button variation="outline" iconPosition="right" title="TEste" small />
      <div style={{ height: 10 }} />
      <Button variation="outline" iconPosition="left" title="TEste" small />
      <div style={{ height: 10 }} />
      <Button variation="outline" iconPosition="centerRight" title="TEste" small />
      <div style={{ height: 10 }} />
      <Button variation="outline" icon={<HeartIcon />} iconPosition="centerleft" title="TEste" small />
      <div style={{ height: 10 }} />
      <Button variation="outline" disabled iconPosition="none" title="TEste" small />
      <div style={{ height: 10 }} />
      <Button variation="outline" disabled iconPosition="right" title="TEste" small />
      <div style={{ height: 10 }} />
      <Button variation="outline" disabled iconPosition="left" title="TEste" small />
      <div style={{ height: 10 }} />
      <Button variation="outline" disabled iconPosition="centerRight" title="TEste" small />
      <div style={{ height: 10 }} />
      <Button variation="outline" disabled icon={<HeartIcon />} iconPosition="centerleft" title="TEste" small />
      <div style={{ height: 10 }} />

      <Button variation="ghost" iconPosition="none" title="TEste" small />
      <div style={{ height: 10 }} />
      <Button variation="ghost" iconPosition="right" title="TEste" small />
      <div style={{ height: 10 }} />
      <Button variation="ghost" iconPosition="left" title="TEste" small />
      <div style={{ height: 10 }} />
      <Button variation="ghost" iconPosition="centerRight" title="TEste" small />
      <div style={{ height: 10 }} />
      <Button variation="ghost" icon={<HeartIcon />} iconPosition="centerleft" title="TEste" small />
      <div style={{ height: 10 }} />
      <Button variation="ghost" disabled iconPosition="none" title="TEste" small />
      <div style={{ height: 10 }} />
      <Button variation="ghost" disabled iconPosition="right" title="TEste" small />
      <div style={{ height: 10 }} />
      <Button variation="ghost" disabled iconPosition="left" title="TEste" small />
      <div style={{ height: 10 }} />
      <Button variation="ghost" disabled iconPosition="centerRight" title="TEste" small />
      <div style={{ height: 10 }} />
      <Button variation="ghost" disabled icon={<HeartIcon />} iconPosition="centerleft" title="TEste" small />
      <div style={{ height: 10 }} />
    </div> */}
    <div style={{ height: 10 }} />
    <Avatar  image imageUrl='data:image/jpeg;base64,'/>
    <div style={{ height: 10 }} />
    <div style={{ height: 10 }} />
    <Avatar image size={36}  imageUrl='data:image/jpeg;base64,'/>
    <div style={{ height: 10 }} />
    <div style={{ height: 10 }} />
    <Avatar image size={24} imageUrl='data:image/jpeg;base64,'/>
    <div style={{ height: 10 }} />


    <div style={{ height: 10 }} />
    <Avatar />
    <div style={{ height: 10 }} />
    <div style={{ height: 10 }} />
    <Avatar size={36} />
    <div style={{ height: 10 }} />
    <div style={{ height: 10 }} />
    <Avatar size={24}/>
    <div style={{ height: 10 }} />

    <div style={{ height: 10 }} />
    <Avatar  icon={<HeartIcon/>}/>
    <div style={{ height: 10 }} />
    <div style={{ height: 10 }} />
    <Avatar size={36}  icon={<HeartIcon width={26} height={26} />}/>
    <div style={{ height: 10 }} />
    <div style={{ height: 10 }} />
    <Avatar size={24} icon={<HeartIcon/>}/>
    <div style={{ height: 10 }} /> 
  </>
);

export default App;
